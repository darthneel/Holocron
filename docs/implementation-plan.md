...
- A consolidated Meetings workspace with scheduling-request and briefing tabs,
  a source-backed editor, version browser, and review controls
...
- `BriefingGeneration` sends the manifest through the task-specific model router
  ...
  generator.
- The model returns five generated sections: overview, attendees, prior history,
  objectives, and logistics. Application code deterministically derives
  `relationship_context` from the selected people, organizations, and prior
  interactions, then attaches its source snapshots before persistence.
- Deterministic validation rejects missing or duplicate model-generated section
  types, uncited material sections, unknown source handles, and source types
  that are irrelevant to a section. `prior_history` additionally rejects
  interactions sourced from the current scheduling request.
...
- Missing prior history and context-limit omissions render in a visible `Known
  limitations` section.
- Relationship context is derived from the bounded manifest rather than
  generated by the model. It is hidden in the workbench when no deterministic
  relationship facts are available; older empty sections remain preserved in
